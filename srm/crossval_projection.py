...
def project_data_crossval(run1_arrs,
    ...
    for trainrun_idx in range(2):
        ...
        # prepare results array for all subjects (shape nsubs, k, nvols) projected data
        # and our shared responses from the training data
        ...
        for testsub_idx in range(len(training_arrs)):
            ...
            trainsubs_traindata = [x for i, x in enumerate(training_arrs) if i != testsub_idx]
            testsub_traindata = training_arrs[testsub_idx]
            # train srm on training subject's training data
            srm = RSRM(n_iter=niter, features=k)
            print('fitting srm')
            srm.fit(trainsubs_traindata)
            # estimate test subject's bases
            print('estimating subjects bases')
            w, s = srm.transform_subject(testsub_traindata)
            ...
            # project test run into shared space
            print('projecting data')
            projected_data, ind_terms = srm.transform(test_arrs)
            # select projected data from test subject
            testsub_proj = projected_data[testsub_idx]
            # append result to our results arrays
            print('saving results')
            results_array[testsub_idx] = testsub_proj
